...
从输入空间\\( \var{X} \subseteq R^n\\)到输出空间\\(\var{Y} \subseteq \{+1, -1\}\\)的函数
$$f(x)=sign(w\*\var{x} + b)$$被称为感知机。输入x表示实例的特征向量，输出y表示实例的类别，w和b为感知机模型的参数，分别叫做权值向量和偏置。\\(sign(x)\\)是符号函数，即
$$sign(x)=
\begin{cases}
+1,&  &{x \geq 0}\\
-1,&  &{ x < 0 }
...
线性方程$$w\*\var{x} + b = 0$$对应于特征空间\\(R^n\\)中的一个超平面，其中\\(w\\)是超平面的法向量，\\(b\\)是超平面的截距。这个超平面恰好将特征空间的点分为两个部分，分别对应于正、负两类实例。

### 数据集的线性可分性

给定一个数据集$$T = \{(x_{1},y_{1}),(x_{2},y_{2}),\cdots, (x_{N},y_{N})},$$其中, \\(x_{i}\\)属于特征空间的点，如果存在某个超平面\\(S\\)$$w \* \var{x} + b = 0$$能够完全将数据集的实例点正确分类即对所有的\\(i\\)都有$$\var{y}_{i}(w \* \var{x_{i}} + b) >0, 则称线性数据集\\(T\\)为线性可分数据集(linearly separable data set); 否则, 称数据集\\(T\\)线性可分。
...
$$L(w, b)= \sum_{i=1}^{N}{\[y_{i}(w \* x_{i} + b)\]_{+}}$$
其中\\(\[x\]_{+}\\)表示取不小于\\(x\\)的最小的非负值，即
$$\[x\]_{+} = \begin{cases}
x, &   &x \gt 0\\
0, &   &0 \leq 0
\end{cases}$$
...
>    (1) 初始化\\(\alpha=0,b=0\\)
>    (2) 在训练数据集中选取（xi, yi）
>    (3) 如果 \\(y_{i}(\sum\limits_{j=1}^{N}{\alpha_{j}y{j}x_{j}\*x_{i}} + b) \leq 0\\)
...
给定线性可分训练数据集，通过间隔最大化或等价地求解相应的凸二次规划问题学习得到的分离超平面$$w^{*} \* x + b ^{*}$$以及相应的分类决策函数$$f(x)=sign(w^{*} \* x + b)$$称为线性可分支持向量机。

### 几何间隔和函数间隔

对于给定的超平面(w, b)及其法向量w, 实例点\\(x_{i}\\)到超平面的距离为:
$$\hat{\gamma_{i}} = \frac{abs{w \* x + b}}{\left \| w \right \|}$$
亦即
$$\hat{\gamma_{i}} = \frac{y_{i}(w \* x + b)}{\left \| w \right \|}$$
因为当被正确分类时, \\(y_{i}\\) 与 \\(w \* x + b\\)的符号是相同的。
取最小的几何间隔为训练数据集到超平面的几何间隔。
这就是几何间隔，当我们不在考虑分类正确与否时，实例点分类错误时几何间隔为负值。

相应的，定义
$$\hat{\gamma_{i}} = y_{i}(w \* x + b)$$
...
&s.t.  &y_{i}(\frac{w}{\left \| w \right \|} \* x_{i} + \frac{b}{\left \| w \right \|}) \geq \gamma, i=1,2,\cdots,N
\end{aligned}$$

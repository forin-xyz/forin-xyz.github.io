...
给定一个数据集$$T = \{(x_{1},y_{1}),(x_{2},y_{2}),\cdot s, (x_{N},y_{N})\},$$其中, \\(x_{i}\\)属于特征空间的点，如果存在某个超平面\\(S\\)$$w \cdot \mathit{x} + b = 0$$能够完全将数据集的实例点正确分类即对所有的\\(i\\)都有$$\mathit{y}_{i}(w \cdot \mathit{x_{i}} + b) >0, 则称线性数据集\\(T\\)为线性可分数据集(linearly separable data set); 否则, 称数据集\\(T\\)线性可分。
...
>输入：\\(T={(x_{1},y_{1}),(x_{2},y_{2}), \cdot s ,(x_{N},y_{N})}\\)（其中\\(x_{i} \in R^n\\)，\\(y_{i}\\)∈Y={-1,+1}，i=1,2...N，学习速率为η）
...
&s.t.  &y_{i}(\frac{w}{\left \| w \right \|} \cdot x_{i} + \frac{b}{\left \| w \right \|}) \geq \gamma, i=1,2,\cdot s,N
\end{aligned}$$
